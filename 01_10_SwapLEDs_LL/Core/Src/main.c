...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  ledState = LL_GPIO_ReadOutputPort(greenLED_GPIO_Port);
	  if(b1State == 1 && last != b1State){
		  ledState ^= greenLED_Pin | redLED_Pin;
	  }
	  LL_GPIO_WriteOutputPort(redLED_GPIO_Port, ledState);
	  LL_mDelay(5);
	  last = b1State;
