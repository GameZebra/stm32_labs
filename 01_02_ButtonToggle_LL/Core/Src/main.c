...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  state = (LL_GPIO_ReadInputPort(userButton_GPIO_Port) & userButton_Pin) >> GPIO_BSRR_BS0_Pos; //the position of the button
	  if(state == 1  && last != state){
		  //first toggle
		  odr = LL_GPIO_ReadOutputPort(greenLED_GPIO_Port);
		  LL_GPIO_WriteOutputPort(greenLED_GPIO_Port, (odr ^ greenLED_Pin) * state);
		  //second toggle
